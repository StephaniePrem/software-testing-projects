...
public class WMethod{
   ...
   */
   public static void main(String [] args){
     ...
     /*Task1 and 2 and part 1 of Task 3
     StringBuilder result = new StringBuilder(); //tests the test cases just made
     String value = result.toString();
     int j=0;
     String temp;
     for(int i = 0; i < tests.size(); i++)
     {
    	 temp = tests.elementAt(i);
    	 while (j<temp.length())
    	 {
    		result = result.append(temp.charAt(j));
        	 if (j == temp.length()-1)
        		break;
     ...
     */
     /*
      * task 3
      * Develop the code, in the WMethod’s main method to output the JUnit methods needed 
      * to test all the generated test cases for a bondRegex method that takes in a string 
      * input and returns a Boolean depending on if the desired sequence was found in the 
      * input string or not
      */
     String end;
     StringBuilder result = new StringBuilder(); //tests the test cases just made
     ...
     for(int i = 0; i < tests.size(); i++)
     {
    	...
    	System.out.println("@Test\npublic void testCase"+u+" () {"); //prints the beginning of the method
    	end =Utilities.runFSM(FSM, 1, value, " ");
    	if ( end.contains("true"))
  	   	{
  		   	System.out.print("assertTrue(x.bondRegex(\""+value+"\"));");
  	   	}	
  	   	else
  	   	{
  	   		System.out.print("assertFalse(x.bondRegex(\""+value+"\"));");
  	   	}
    	System.out.println("\n}");
    	u++;
