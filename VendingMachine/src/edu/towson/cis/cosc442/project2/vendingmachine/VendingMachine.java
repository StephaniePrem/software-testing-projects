...
public class VendingMachine {
	...
	private static final String INVALID_AMOUNT_MESSAGE = " Invalid amount.  Amount must be > 0"; //changed to > 0
	
	//Error message for balance < 0
	 ...
	 */
	protected VendingMachineItem getItem(String code) throws VendingMachineException { //needs an if statement
		int slotIndex = getSlotIndex(code);//added this.
		/*
		if ( itemArray[slotIndex] != null) {
			throw new VendingMachineException(SLOT_MESSAGE + code + INVALID_CODE_MESSAGE);
		}
		*/
		if ( slotIndex > 4) { //checks if the code is not A, B, C, or D
			throw new VendingMachineException(SLOT_MESSAGE + code + INVALID_CODE_MESSAGE);
		}
	...
	public VendingMachineItem removeItem(String code) throws VendingMachineException {
		...
		if (itemArray[slotIndex] != null) {//needs to test if the code is invalid
			throw new VendingMachineException(SLOT_MESSAGE + code + INVALID_CODE_MESSAGE);
		}
		return item;
	}
	...
	public void insertMoney(double amount) throws VendingMachineException {
		if( amount <= 0 )//should be <=0, $0.00 cannot be entering into the machine 
	 ...
	 */
	public double getBalance() throws VendingMachineException{//change: code now throws exception if the balance is less than 0
		if( this.balance < 0 )
